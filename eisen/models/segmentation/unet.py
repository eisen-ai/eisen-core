...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, normalization_fn):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2), DoubleConv(in_channels, out_channels, normalization_fn)
        )
...
class UNet(nn.Module):
    def __init__(
        ...
        self.up1 = Up(
            self.n_filters * 16, self.n_filters * 4, normalization_fn, bilinear
        )
        self.up2 = Up(
            self.n_filters * 8, self.n_filters * 2, normalization_fn, bilinear
        )

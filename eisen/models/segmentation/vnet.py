...
class VNet(nn.Module):
    def __init__(
        ...
        if input_channels > 1:
            ...
            self.block_one = ResidualConvBlock(
                1, input_channels, n_filters, normalization=normalization
            )
        ...
        self.block_two = ResidualConvBlock(
            filter_size, n_filters * 2, n_filters * 2, normalization=normalization
        )
        self.block_two_dw = DownsamplingConvBlock(
            n_filters * 2, n_filters * 4, normalization=normalization
        )

        self.block_three = ResidualConvBlock(
            filter_size, n_filters * 4, n_filters * 4, normalization=normalization
        )
        self.block_three_dw = DownsamplingConvBlock(
            n_filters * 4, n_filters * 8, normalization=normalization
        )

        self.block_four = ResidualConvBlock(
            filter_size, n_filters * 8, n_filters * 8, normalization=normalization
        )
        self.block_four_dw = DownsamplingConvBlock(
            n_filters * 8, n_filters * 16, normalization=normalization
        )

        self.block_five = ResidualConvBlock(
            filter_size, n_filters * 16, n_filters * 16, normalization=normalization
        )
        self.block_five_up = UpsamplingDeconvBlock(
            n_filters * 16, n_filters * 8, normalization=normalization
        )

        self.block_six = ResidualConvBlock(
            filter_size, n_filters * 8, n_filters * 8, normalization=normalization
        )
        ...
        self.block_seven = ResidualConvBlock(
            filter_size, n_filters * 4, n_filters * 4, normalization=normalization
        )
        ...
        self.block_eight = ResidualConvBlock(
            filter_size, n_filters * 2, n_filters * 2, normalization=normalization
        )
        ...
        self.block_nine = ResidualConvBlock(

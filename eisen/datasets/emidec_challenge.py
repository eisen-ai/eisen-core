...
class EMIDEC(Dataset):
    ...
    def __init__(self, data_dir, training, transform=None):
        ...
        directories = [
            ...
            if os.path.isdir(os.path.join(data_dir, d)) and ("." not in d)
        ]
        ...
        for directory in directories:
            ...
            with open(
                os.path.join(data_dir, directory.replace("_", " ") + ".txt"),
                "r",
                encoding="utf-8",
                errors="ignore",
            ) as file:
            ...
            if self.training:
                label_rel_path = os.path.join(
                    directory, "Contours", directory + ".nii.gz"
                )

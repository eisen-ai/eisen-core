...
class SaveTorchModel:
    ...
    def __init__(self, artifacts_dir):
        ...
        if not os.path.exists(self.artifacts_dir):
            raise ValueError(
                "The artifacts directory passed as parameter to the SaveTorchModel"
...
class SaveTorchModelHook:
    ...
    def __init__(
        ...
        if select_best_loss:
            dispatcher.connect(
                self.save_model, signal=EISEN_BEST_MODEL_LOSS, sender=workflow_id
            )
        else:
            dispatcher.connect(
                self.save_model, signal=EISEN_BEST_MODEL_METRIC, sender=workflow_id
            )

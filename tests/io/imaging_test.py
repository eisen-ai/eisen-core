...
class TestLoadNiftiFromFilename:
    def setup_class(self):
        ...
        self.dset = {"nii": self.nii_name, "nii_gz": self.nii_gz_name}
...
class TestLoadPILImageFromFilename:
    def setup_class(self):
        ...
        self.data_entry = {"png": self.png_name}
    ...
    def test_call(self):
        ...
        assert isinstance(
            data_entry_transformed["png"], PIL.PngImagePlugin.PngImageFile
        )
...
class TestLoadITKImageFromFilename:
    def setup_class(self):
        ...
        self.data_entry = {"mhd": self.mhd_name}
    ...
    def test_call(self):
        ...
        assert np.all(
            self.data == sitk.GetArrayFromImage(data_entry_transformed["mhd"])
        )
...
class TestLoadDICOMImageFromFilename:
    def setup_class(self):
        ...
        self.data_entry = {"dcm": self.dcm_name}

        self.loader = LoadDICOMFromFilename(["dcm"], self.base_path)
        self.loader_px_data = LoadDICOMFromFilename(
            ["dcm"], self.base_path, store_data_array=True
        )

    def test_call(self):
        data_entry_transformed = self.loader(copy.deepcopy(self.data_entry))

        data_entry_transformed_with_data_array = self.loader_px_data(
            copy.deepcopy(self.data_entry)
        )
        ...
        assert isinstance(
            data_entry_transformed_with_data_array["dcm_pixel_array"], np.ndarray
        )

        assert np.all(
            self.data == np.asanyarray(data_entry_transformed["dcm"].pixel_array)
        )
        assert np.all(
            self.data
            == np.asanyarray(data_entry_transformed_with_data_array["dcm_pixel_array"])
        )
        assert np.all(
            self.data
            == np.asanyarray(data_entry_transformed_with_data_array["dcm"].pixel_array)
        )
...
class TestWriteNiftiToFile:
    def setup_class(self):
        ...
        self.affine = np.array(
            [[1, 0, 0, -100], [0, 1, 0, -200], [0, 0, 2.5, -300], [0, 0, 0, 1]]
        )
        self.base_path = tempfile.mkdtemp()

    def test_call(self):
        nifti_tform = NumpyToNifti(
            self.data, affine=self.affine, data_types=self.data_types
        )

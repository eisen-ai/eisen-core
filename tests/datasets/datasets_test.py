...
class TestPatchCamelyon:
    def setup_class(self):
        ...
        self.camelyon_dset = PatchCamelyon(
            self.base_path, self.file_name_x, self.file_name_y
        )
...
class TestCAMUS:
    def setup_class(self):
        ...
        touch(
            os.path.join(self.base_path, "patient0001", "patient0001_4CH_sequence.mhd")
        )
        touch(
            os.path.join(self.base_path, "patient0001", "patient0001_2CH_sequence.mhd")
        )
    ...
    def test_getitem(self):
        ...
        assert item["image_2CH"] == str(
            os.path.join("patient0001", "patient0001_2CH_ED.mhd")
        )
        assert item["image_4CH"] == str(
            os.path.join("patient0001", "patient0001_4CH_ED.mhd")
        )

        assert item["label_2CH"] == str(
            os.path.join("patient0001", "patient0001_2CH_ED_gt.mhd")
        )
        assert item["label_4CH"] == str(
            os.path.join("patient0001", "patient0001_4CH_ED_gt.mhd")
        )

        assert item["sequence_2CH"] == str(
            os.path.join("patient0001", "patient0001_2CH_sequence.mhd")
        )
        assert item["sequence_4CH"] == str(
            os.path.join("patient0001", "patient0001_4CH_sequence.mhd")
        )
        ...
        assert item["image_2CH"] == str(
            os.path.join("patient0001", "patient0001_2CH_ES.mhd")
        )
        assert item["image_4CH"] == str(
            os.path.join("patient0001", "patient0001_4CH_ES.mhd")
        )

        assert item["label_2CH"] == str(
            os.path.join("patient0001", "patient0001_2CH_ES_gt.mhd")
        )
        assert item["label_4CH"] == str(
            os.path.join("patient0001", "patient0001_4CH_ES_gt.mhd")
        )
...
class TestRSNABoneAgeChallenge:
    def setup_class(self):
        ...
        training_img_dir = os.path.join(
            self.base_path, "boneage-training-dataset", "boneage-training-dataset"
        )
        test_img_dir = os.path.join(
            self.base_path, "boneage-test-dataset", "boneage-test-dataset"
        )
        ...
        with open(
            os.path.join(self.base_path, "boneage-training-dataset.csv"), mode="w"
        ) as metadata:
        ...
        with open(
            os.path.join(self.base_path, "boneage-test-dataset.csv"), mode="w"
        ) as metadata:
    ...
    def test_getitem(self):
        for item in self.dataset_train:
            ...
            ) or item["image"] == os.path.join(
                "boneage-training-dataset", "boneage-training-dataset", "12346.png"
            )

            if item["image"] == os.path.join(
                "boneage-training-dataset", "boneage-training-dataset", "12345.png"
            ):
        ...
        for item in self.dataset_test:
            assert item["image"] == os.path.join(
                "boneage-test-dataset", "boneage-test-dataset", "11111.png"
            ) or item["image"] == os.path.join(
                "boneage-test-dataset", "boneage-test-dataset", "22222.png"
            )

            if item["image"] == os.path.join(
                "boneage-training-dataset", "boneage-training-dataset", "11111.png"
            ):
                assert item["male"]

            if item["image"] == os.path.join(
                "boneage-training-dataset", "boneage-training-dataset", "22222.png"
            ):
                assert not item["male"]
...
class TestRSNAIntracranialHemorrhageDetection:
    def setup_class(self):
        ...
        with open(
            os.path.join(self.base_path, "stage_2_train.csv"), mode="w"
        ) as metadata:
            fieldnames = ["ID", "Label"]
            writer = csv.DictWriter(metadata, fieldnames=fieldnames)
        ...
        self.dataset_train = RSNAIntracranialHemorrhageDetection(
            self.base_path, training=True
        )

        self.dataset_test = RSNAIntracranialHemorrhageDetection(
            self.base_path, training=False
        )
    ...
    def test_getitem(self):
        for item in self.dataset_train:
            assert item["image"] == os.path.join(
                "stage_2_train", "ID_000012eaf.dcm"
            ) or item["image"] == os.path.join("stage_2_train", "ID_000039fa0.dcm")
        ...
        for item in self.dataset_test:
            assert item["image"] == os.path.join(
                "stage_2_test", "ID_000000e27.dcm"
            ) or item["image"] == os.path.join("stage_2_test", "ID_000009146.dcm")
...
class TestMSDDataset:
    def setup_class(self):
        ...
        self.dataset_train = MSDDataset(
            self.base_path, "json_file.json", phase="training"
        )
...
class TestPANDA:
    def setup_class(self):
        ...
        filenames = [
            ...
            os.path.join(
                self.labels_path, "0005f7aaab2800f6170c399693a96917_mask.tiff"
            ),
            os.path.join(
                self.labels_path, "000920ad0b612851f8e01bcc880d9b3d_mask.tiff"
            ),
            os.path.join(
                self.labels_path, "0018ae58b01bdadc8e347995b69f99aa_mask.tiff"
            ),
        ]
    ...
    def test_training_set(self):
        ...
        assert element["image"] == os.path.join(
            "train_images", "0005f7aaab2800f6170c399693a96917.tiff"
        )
        assert element["mask"] == os.path.join(
            "train_label_masks", "0005f7aaab2800f6170c399693a96917_mask.tiff"
        )
...
class TestBrats2020:
    def setup_class(self):
        ...
        with open(
            os.path.join(patient_dir, "BraTS20_Training_001_t1.nii.gz"), "w"
        ) as f:
            f.write("")

        with open(
            os.path.join(patient_dir, "BraTS20_Training_001_t2.nii.gz"), "w"
        ) as f:
            f.write("")

        with open(
            os.path.join(patient_dir, "BraTS20_Training_001_flair.nii.gz"), "w"
        ) as f:
            f.write("")

        with open(
            os.path.join(patient_dir, "BraTS20_Training_001_t1ce.nii.gz"), "w"
        ) as f:
            f.write("")

        with open(
            os.path.join(patient_dir, "BraTS20_Training_001_seg.nii.gz"), "w"
        ) as f:
            f.write("")
    ...
    def test_training_dataset(self):
        ...
        assert all(
            r in dataset[0]["survival_info"].keys()
            for r in self.expected_survivalinfo_keys
        )
        assert all(
            r in dataset[0]["name_mapping"].keys()
            for r in self.expected_namemapping_keys
        )

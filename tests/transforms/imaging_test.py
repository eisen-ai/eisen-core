...
class TestCreateConstantFlags:
    def setup_class(self):
        self.data = {"image": np.random.rand(32, 32, 3), "label": 1}

        self.tform_one = CreateConstantFlags(["flag1", "flag2"], [32.2, 42.0])
        self.tform_two = CreateConstantFlags(
            ["flag3", "flag4", "flag5"], ["flag3", 42, False]
        )
...
class TestRenameFields:
    def setup_class(self):
        self.data = {"image": np.ones([32, 32, 3], dtype=np.float32), "label": 0}
...
class TestResampleNiftiVolumes:
    def setup_class(self):
        ...
        self.data = {"image": img, "label": 0}

        self.tform_one = ResampleNiftiVolumes(
            ["image"], [0.5, 0.5, 0.5], interpolation="linear"
        )
        self.tform_two = ResampleNiftiVolumes(
            ["image"], [1.0, 1.0, 1.0], interpolation="linear"
        )
        self.tform_three = ResampleNiftiVolumes(
            ["image"], [2.0, 2.0, 2.0], interpolation="linear"
        )
...
class TestCropCenteredSubVolumes:
    def setup_class(self):
        ...
        self.data = {"image": self.data_one, "other": self.data_two}
